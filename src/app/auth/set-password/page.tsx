...
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        ...
        } else if (data.code === 'PASSWORD_ALREADY_SET') {
          setTokenStatus('already_set')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await jsonFetcher<{ success?: boolean; data?: { email: string }; error?: string; message?: string }>('/api/auth/set-password', {
        method: 'POST',
      ...
      if (data.success || data.data?.email) {
        ...
        if (signInResult?.ok) {
          ...
          router.push('/auth/signin?from=set-password')
        }
      } else {
      ...
      <AuthSplitLayout
        left={
          <div className="relative group">
            ...
            <div className="relative p-12 lg:p-14">
              <h1 className="text-5xl lg:text-6xl font-display font-bold text-slate-900 mb-6">{t('alreadySetTitle')}</h1>
              <p className="text-xl lg:text-2xl text-slate-600 mb-12">{t('alreadySetDescription')}</p>
            </div>
          </div>
        }
      >
        <AuthCard title={t('alreadySet')}>
          <div className="space-y-6 text-center">
            <p className="text-slate-600">{t('alreadySetHelp')}</p>
            <div className="pt-4">
              <Link 
                href="/auth/signin" 
                className="inline-flex items-center justify-center px-6 py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition-colors"
