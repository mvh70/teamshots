...
import { refundCreditsForFailedGeneration } from '@/lib/credits'
...
const BUCKET_NAME = process.env.HETZNER_S3_BUCKET || process.env.S3_BUCKET_NAME!
...
const imageGenerationWorker = new Worker<ImageGenerationJobData>(
  ...
  async (job: Job<ImageGenerationJobData>) => {
    ...
    try {
      console.log(`Starting image generation for job ${job.id}, generation ${generationId}`)
      ...
      console.log('🔍 Using original selfie without background removal processing')
      ...
      // Debug logging
      console.log('🔍 Generation context:', generation.context?.name)
      console.log('🔍 Context settings:', JSON.stringify(generation.context?.settings, null, 2))
      console.log('🔍 Job styleSettings:', JSON.stringify(styleSettings, null, 2))
      console.log('🔍 Final styleSettings:', JSON.stringify(finalStyleSettings, null, 2))
      ...
      try {
        ...
        console.log(`✅ Credits refunded for failed generation ${generationId}`)
      } catch (refundError) {
        console.error(`❌ Failed to refund credits for generation ${generationId}:`, refundError)
      }
...
imageGenerationWorker.on('failed', (job, error) => {
  if (!job) {
    console.error('❌ Job (unknown) failed:', error)
    return
  }
  console.error(`❌ Job ${job.id} failed with ${job.attemptsMade} attempts:`, error)
})
...
async function createCompositeImage(styleSettings: {
  ...
  console.log('🔍 Creating composite image for selfie:', selfieS3Key)
  const selfie = await downloadSelfieAsBase64(selfieS3Key)
  console.log('🔍 Selfie downloaded, size:', selfie.base64.length, 'chars')
  ...
  console.log('🔍 Selfie buffer size:', selfieBuffer.length, 'bytes')
  
  const selfieSharp = sharp(selfieBuffer)
  const selfieMetadata = await selfieSharp.metadata()
  console.log('🔍 Selfie original dimensions:', selfieMetadata.width, 'x', selfieMetadata.height)
  
  // Use original selfie buffer without additional processing to preserve quality
  const selfieProcessed = selfieBuffer
  console.log('🔍 Using original selfie buffer size:', selfieProcessed.length, 'bytes')
  ...
  console.log('🔍 Added SUBJECT at Y:', currentY - (selfieMetadata.height || 0) - labelHeight - spacing)
  
  // Add background section if custom
  if (styleSettings?.background?.type === 'custom' && styleSettings?.background?.key) {
    console.log('🔍 Adding custom background section')
    ...
    if (bg) {
      ...
      console.log('🔍 Background original dimensions:', bgMetadata.width, 'x', bgMetadata.height)
      ...
      console.log('🔍 Added BACKGROUND at Y:', currentY - (bgMetadata.height || 0) - labelHeight - spacing)
    }
  }
  
  // Add logo section if included
  if (styleSettings?.branding?.type === 'include' && styleSettings?.branding?.logoKey) {
    console.log('🔍 Adding logo section')
    ...
    if (logo) {
      ...
      console.log('🔍 Logo original dimensions:', logoMetadata.width, 'x', logoMetadata.height)
      ...
      console.log('🔍 Added LOGO at Y:', currentY - (logoMetadata.height || 0) - labelHeight - spacing)
    }
  }
  ...
  console.log('🔍 Calculated canvas size:', compositeWidth, 'x', compositeHeight)
  console.log('🔍 Creating composite with', compositeElements.length, 'elements')
  ...
  console.log('🔍 Composite image created, size:', compositeBase64.length, 'chars')
  console.log('🔍 Composite buffer size:', compositeBuffer.length, 'bytes')
