/**
 * Industry-specific styling configuration
 *
...
export const INDUSTRY_STYLE_CONFIGS: Record<IndustryType, IndustryStyleConfig> = {
  'law-firms': {
    background: {
      type: 'office',
      prompt:
...
/**
 * Get industry style config with fallback to law-firms
 */
export function getIndustryStyleConfig(industry: string | undefined): IndustryStyleConfig {
  if (industry && industry in INDUSTRY_STYLE_CONFIGS) {
    return INDUSTRY_STYLE_CONFIGS[industry as IndustryType]
  }
  return INDUSTRY_STYLE_CONFIGS['law-firms']
}
...
export function isValidIndustry(industry: string): industry is IndustryType {
  return industry in INDUSTRY_STYLE_CONFIGS
}

/**
 * List of all available industries
 */
export const AVAILABLE_INDUSTRIES: IndustryType[] = Object.keys(INDUSTRY_STYLE_CONFIGS) as IndustryType[]
