...
export class CameraSettingsElement extends StyleElement {
  ...
  async contribute(context: ElementContext): Promise<ElementContribution> {
    ...
    // Note: For person-generation, focus requirement is in Technical Requirements
    // and camera settings (focal_length, aperture, positioning) are in the JSON payload
    if (phase === 'background-generation') {
      ...
      // Composition phase: DOF and depth instructions are handled by the hardcoded
      // "Depth & Spatial Composition" section in v3-step2-final-composition.ts.
      // The camera JSON payload already specifies aperture, positioning, and subject_to_background_ft.
