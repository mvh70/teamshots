// Prompt builders for image generation

// export function buildPromptFromStyleSettings(style: any): string {
//   const parts: string[] = []
//   if (style?.background?.type === 'office') parts.push('Professional office background.')
//   if (style?.background?.prompt) parts.push(`Background detail: ${style.background.prompt}.`)
//   if (style?.branding?.type === 'include') parts.push('Include brand logo in a tasteful, unobtrusive way.')
//   if (style?.style?.preset) parts.push(`Overall style preset: ${style.style.preset}.`)
//   if (style?.clothing?.type) parts.push(`Clothing: ${style.clothing.type}.`)
//   if (style?.expression?.type) parts.push(`Facial expression: ${style.expression.type}.`)
//   if (style?.lighting?.type) parts.push(`Lighting: ${style.lighting.type}.`)
//   parts.push('Generate a realistic professional headshot suitable for business use.')
//   return parts.join(' ')
// }

// Structured prompt builder: emits many optional fields but only includes ones
// present in the provided photo style. If a field is not determined from style,
// it is omitted from the JSON to avoid over-constraining the model while keeping
...
export function buildStructuredPromptFromStyle(style: Record<string, unknown>, basePrompt?: string): string {
  ...
  // Branding → scene.environment.branding
  ...
  // Style preset → scene.mood / rendering_intent
  ...
  // Expression → subject.pose.expression
  ...
  // Business headshot oriented minimal defaults
  ...
  // Build the final object conditionally adding sections only if they have keys
  ...
  // Allow passing user prompt as additional description if provided
  if (basePrompt) {
