...
export async function POST(request: NextRequest) {
  try {
    ...
    const isPersonalContext = contextType === 'personal' || (!contextType && !teamId)
    ...
    if (isPersonalContext) {
      ...
      // Create team context (requires user to be part of a team)
      if (!teamId) {
        return NextResponse.json({ 
          error: 'User must be part of a team to create team contexts' 
        }, { status: 400 })
      }
